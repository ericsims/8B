...
def validate_opcode(op):
    if op['opcode'] < int(0x00) or op['opcode'] > int(0x7F):
        print(colored('\t\t***ERROR: opcode 0x{:02X} is out of range 0x00 - 0x7F'. format(op['opcode']),'red'))
        exit(-1)
    ...
    for uc in op['ucode']:
        for iv in invalid_control_signals:
            if iv['condition'].issubset(set(uc)):
                print(colored('\t\t***ERROR: {}'.format(iv['error']),'red'))
                exit(-1)
    
    # check for duplicates in ucode
    for uc in op['ucode']:
        if len(uc) != len(set(uc)):
            print(colored('\t\t***ERROR: duplicate control signals in ucode line: {}'.format(uc),'red'))
            exit(-1)
...
with open('instruction_set.yaml', 'r') as stream:
    try:
        ...
        print(tabulate((inst_list[i:i+4] for i in range(0, len(inst_list), 4)), tablefmt="fancy_grid"))
